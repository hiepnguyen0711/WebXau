    ...
    if ($_REQUEST['a'] == 'man') {
        if (isset($_GET['search']) and $_GET['key'] != ''  and $_GET['search'] != '') {
            if ($_GET['search'] == 'loai') {
                ...
                $loai .= $d->array_category(0, '', $id_code, 29);
            } else {
        ...
        if (isset($_REQUEST['id'])) {
            ...
            $loai = $d->array_category(0, '', $items[0]['id_loai'], 2);
            $loai .= $d->array_category(0, '', $items[0]['id_loai'], 29);
        }
    }
}
    ...
    if ($d->checkPermission_edit($id_module) == 1) {
        ...
        if ($id != '') {
            ...
            if ($d->update($data0)) {
                //upload hình album
                $arr_img = $_POST['album'];
                $d->o_que("delete from #_tintuc_hinhanh where id_tintuc = $id");
                // var_dump($arr_img);
                // die();
                for ($i = 0; $i < count($arr_img); $i++) {
                    unset($data_img);
                    $data_img['id_tintuc'] = $id;
                    $data_img['hinh_anh'] = $arr_img[$i];
                    $data_img['stt'] = $_POST['album_stt'][$i];
                    // var_dump($data_img);
                    // die();
                    $d->reset();
                    $d->setTable('#_tintuc_hinhanh');
                    $d->insert($data_img);
                }
                ...
                foreach (get_json('lang') as $key => $value) {
                    ...
                    $data['sanpham_kem']    = ',' . implode(',', $_POST['sanpham_kem']) . ',';
                    //xu ly tags
