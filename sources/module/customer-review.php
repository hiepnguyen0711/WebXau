...
if (isset($_POST['guibinhluan'])) {
    ...
    if ($id = $d->insert($data)) {
        ...
        $thongbao_content   =   'Cảm ơn bạn đã đánh giá';
...
$namsao = $d->num_rows("select id from #_binhluan where id_tin  =   " . (int)$row['id_code'] . " and trang_thai = 1 and parent=0 and danh_gia=5 ");
$bonsao = $d->num_rows("select id from #_binhluan where id_tin  =   " . (int)$row['id_code'] . " and trang_thai = 1 and parent=0 and danh_gia=4 ");
$basao = $d->num_rows("select id from #_binhluan where id_tin   =   " . (int)$row['id_code'] . " and trang_thai = 1 and parent=0 and danh_gia=3 ");
$haisao = $d->num_rows("select id from #_binhluan where id_tin  =   " . (int)$row['id_code'] . " and trang_thai = 1 and parent=0 and danh_gia=2 ");
$motsao = $d->num_rows("select id from #_binhluan where id_tin  =   " . (int)$row['id_code'] . " and trang_thai = 1 and parent=0 and danh_gia=1 ");
?>
...
<div class="danh-gia" id="nhanxet">
    <div class="row m-10">
        <div class="col-md-3 col-sm-5 p10">
            <div class="rating-medium">
                ...
                <div class="quantity-star">
                    <?php for ($i = 0; $i < round($sao_trung_binh); $i++) { ?>
                        <i class="fas fa-star"></i>
                    <?php } ?>
                    <?php for ($i = 0; $i < 5 - round($sao_trung_binh); $i++) { ?>
        ...
        <div class="col-md-3 col-sm-12 p10 text-center">
            <p>Chia sẻ nhận xét về bài viết</p>
    ...
    <div class="content-nhanxet" id="main-nx">
        <form method="POST" action="">
            <p style="font-weight: 400;font-size: 16px;">Đánh giá cả bạn về bài viết này</p>
            <div id="cate-rating" class="cate-rating">
                <div class="stars">
                    <a id="star-1" data="1" class="star vote-active"><i class="fas fa-star"></i></a>
                    <a id="star-2" data="2" class="star vote-active"><i class="fas fa-star"></i></a>
                    <a id="star-3" data="3" class="star vote-active"><i class="fas fa-star"></i></a>
                    <a id="star-4" data="4" class="star vote-active"><i class="fas fa-star"></i></a>
                    <a id="star-5" data="5" class="star vote-active"><i class="fas fa-star"></i></a>
                </div>
                <div class="clearfix"></div>
            ...
            <div class="form-group mb-3">
                <textarea class="form-control" rows="5" name="noi_dung" placeholder="Nhận xét của bạn về bài viết này"></textarea>
            </div>
