<?php
$total_records = $d->num_rows("select * from #_sanpham where tieu_bieu = 1 and hien_thi =1 "._where_lang." order by so_thu_tu ASC, id DESC");
$limit = get_json('product', 'paging');
$total_page = ceil($total_records / $limit);
$sanpham = $d->o_fet("select * from #_sanpham where tieu_bieu = 1 and hien_thi =1 "._where_lang." order by so_thu_tu ASC, id DESC limit 0,$limit ");
?>
        ...
        $('#pagination-ajax').twbsPagination({
            ...
            onPageClick: function (event, page) {
                $.ajax({
                    ...
                    type:'POST',
                    data: {page: page,totalPages:'<?=$total_page?>', chuyenmuc: chuyenmuc, limit: '<?=$limit?>', do: 'pagination_sanpham_banchay'},
